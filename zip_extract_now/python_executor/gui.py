...
class ArbApp:
    ...
    def _apply_settings(self):
        try:
            ...
            self.log(f"Settings applied | Entry>{self.live['entry_th']} Exit<{self.live['exit_th']} Lot={self.live['lot']}")
        except Exception as e:
    ...
    def _open_pair(self, cfg, diff, master_spread, slave_spread):
        ...
        master_comment = f"arb_{pair['id']}_master"
        slave_comment = f"arb_{pair['id']}_slave"
    ...
    def _arb_loop(self):
        ...
        global master_ticket, slave_ticket
        in_trade = False
        cfg      = dict(self.live)

        try:
            s, core_host, core_port = connect_core_socket(connect_timeout=5.0)
            self.log(f"Connected to Rust core on {core_host}:{core_port}.")
        except socket.timeout:
            self.log("ERROR: Rust core connection timeout (5s) - is it running?")
            self._stop()
        ...
        while running:
            ...
            try:
                ...
                if in_trade:
                    m_open = is_position_open(master_path, master_ticket)
                    s_open = is_position_open(slave_path,  slave_ticket)

                    if not m_open and s_open:
                        self.log("Master closed externally → closing slave")
                        close_order(slave_path, slave_ticket, cfg["ssym"], cfg["s_dir"], cfg["lot"])
                        in_trade = False
                        self.status_var.set("RUNNING")
                    elif not s_open and m_open:
                        self.log("Slave closed externally → closing master")
                        close_order(master_path, master_ticket, cfg["msym"], cfg["m_dir"], cfg["lot"])
                        in_trade = False
                        self.status_var.set("RUNNING")
                    elif not m_open and not s_open:
                        self.log("Both positions closed")
                        in_trade = False
                        self.status_var.set("RUNNING")

                    if in_trade and diff < cfg["exit_th"]:
                        self.log(f"EXIT diff={diff:.5f} < {cfg['exit_th']} → closing both")
                        close_order(master_path, master_ticket, cfg["msym"], cfg["m_dir"], cfg["lot"])
                        close_order(slave_path,  slave_ticket,  cfg["ssym"], cfg["s_dir"], cfg["lot"])
                        in_trade = False
                        self.status_var.set("RUNNING")
                ...
                if diff > cfg["entry_th"] and not in_trade:
                    ms = get_spread(master_path, cfg["msym"])
                    ss = get_spread(slave_path,  cfg["ssym"])
                    ...
                    self.log(f"ENTRY diff={diff:.5f} M.Spd={ms} S.Spd={ss}")

                    # Add delay between order attempts to avoid API issues
                    import time
                    time.sleep(0.5)

                    m_result = place_order(master_path, cfg["msym"], cfg["m_dir"], cfg["lot"])

                    if not trade_result_ok(m_result):
                        self.log(f"Master FAILED: {trade_result_message(m_result)}")
                        time.sleep(1.0)
                        continue

                    master_ticket = m_result.order
                    self.log(f"Master {self.master_dir_var.get()} #{master_ticket}")

                    s_result = place_order(slave_path, cfg["ssym"], cfg["s_dir"], cfg["lot"])

                    if not trade_result_ok(s_result):
                        if isinstance(s_result, dict) and "error" in s_result:
                            self.log(f"Slave FAILED: {s_result['error']} | {s_result.get('comment', 'Unknown error')} → closing master")
                        elif hasattr(s_result, 'retcode') and hasattr(s_result, 'comment'):
                            self.log(f"Slave FAILED code={s_result.retcode} | {s_result.comment} → closing master")
                        else:
                            self.log(f"Slave FAILED: Unknown error - {type(s_result)} - {s_result} → closing master")
                        close_order(master_path, master_ticket, cfg["msym"], cfg["m_dir"], cfg["lot"])
                        time.sleep(1.0)
                        continue

                    slave_ticket = s_result.order
                    self.log(f"Slave {self.slave_dir_var.get()} #{slave_ticket}")
                    in_trade = True
                    self.status_var.set("IN TRADE")
