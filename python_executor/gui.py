...
MANUAL_MAGIC_BASE = 87000000

# ── Global MT5 connection lock ─────────────────────────
# mt5.initialize() is a process-wide singleton — only one terminal can be
# connected at a time.  This lock serialises all MT5 API access so that
# concurrent threads (auto engine + multiple manual pairs) do not steal
...
def wait_for_position_ticket(path, symbol, ticket=None, magic=None, comment=None, timeout=3.0):
...
def wait_until_position_closed(path, ticket, timeout=3.0):
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
            with _mt5_session(path):
...
class ArbApp:
    def __init__(self, root):
        ...
        self.manual_exit_diff_var = tk.StringVar(value="5.00")
        self.manual_lot_var = tk.StringVar(value="0.01")
        self.manual_enabled_var = tk.IntVar(value=1)
        ...
        self.m_pnl_var = tk.StringVar(value="$—")
        self.s_pnl_var = tk.StringVar(value="$—")
        self.c_pnl_var = tk.StringVar(value="$—")
        self.pnl_threshold_var = tk.StringVar(value="50")

        self.live = {}
        self.active_pair = None
        self.pair_seq = 0

        # Manual trading supports multiple concurrent pairs
        self.manual_pairs = {}          # pair_id -> pair_state
        self.manual_pair_seq = 0
    ...
    def _build_ui(self):
        ...
        tk.Frame(sb, bg=BORDER, width=1, height=18).pack(side="left", padx=2)
        stat("M.PNL:", self.m_pnl_var, GREEN)
        tk.Frame(sb, bg=BORDER, width=1, height=18).pack(side="left", padx=2)
        stat("S.PNL:", self.s_pnl_var, GREEN)
        tk.Frame(sb, bg=BORDER, width=1, height=18).pack(side="left", padx=2)
        stat("COMB:", self.c_pnl_var, YELLOW)
        ...
        tk.Label(i, text="MASTER SYMBOL", bg=CARD, fg=TEXT2,
                 font=("Consolas", 8)).pack(anchor="w")
        self.master_sym_combo = self._combo(i, [], self.master_sym_var, cmd=self._on_symbol_changed)
        self.master_sym_combo.pack(anchor="w", pady=(2, 8))
        tk.Label(i, text="SLAVE SYMBOL", bg=CARD, fg=TEXT2,
                 font=("Consolas", 8)).pack(anchor="w")
        self.slave_sym_combo = self._combo(i, [], self.slave_sym_var, cmd=self._on_symbol_changed)
        ...
        # PnL threshold row
        pnl_row = tk.Frame(i, bg=CARD)
        pnl_row.pack(fill="x", pady=(8, 0))
        tk.Label(pnl_row, text="PNL CLOSE ($ >| |)", bg=CARD, fg=TEXT2,
                 font=("Consolas", 8), width=18, anchor="w").pack(side="left")
        self._entry(pnl_row, self.pnl_threshold_var, width=8).pack(side="left")
        self.close_all_btn = tk.Button(pnl_row, text="!! CLOSE ALL",
            bg=RED, fg="#ffffff", font=("Consolas", 8, "bold"),
            relief="flat", padx=8, pady=4, cursor="hand2",
            activebackground="#cc224f", command=self._close_all_trades)
        self.close_all_btn.pack(side="left", padx=(6, 0))

        # 05b Manual Trade
        o, i = self._card(right, "05B / MANUAL TRADE")
        o.pack(fill="x", pady=(0, 7))

        # Directions
        df = tk.Frame(i, bg=CARD)
        df.pack(fill="x", pady=4)
        tk.Label(df, text="MAN MASTER DIR", bg=CARD, fg=TEXT2,
                 font=("Consolas", 8), width=14, anchor="w").pack(side="left")
        self.manual_master_dir_var = tk.StringVar(value="BUY")
        self._combo(df, ["BUY", "SELL"], self.manual_master_dir_var, width=6).pack(side="left")

        tk.Label(df, text="   SLAVE DIR", bg=CARD, fg=TEXT2,
                 font=("Consolas", 8)).pack(side="left", padx=(6, 2))
        self.manual_slave_dir_var = tk.StringVar(value="SELL")
        self._combo(df, ["BUY", "SELL"], self.manual_slave_dir_var, width=6).pack(side="left")

        # Close rule
        self._row(i, "MAN CLOSE DIFF ( < )", self.manual_exit_diff_var, hint="AUTO-CLOSE")

        # Manual lot
        self._row(i, "MANUAL LOT", self.manual_lot_var)

        # Manual ON/OFF toggle (disable only PLACE)
        toggle_row = tk.Frame(i, bg=CARD)
        toggle_row.pack(fill="x", pady=(6, 0))
        tk.Label(toggle_row, text="MANUAL ON", bg=CARD, fg=TEXT2,
                 font=("Consolas", 8), width=24, anchor="w").pack(side="left")
        self.manual_enabled_chk = tk.Checkbutton(
            toggle_row,
            text="ENABLED",
            variable=self.manual_enabled_var,
            onvalue=1,
            offvalue=0,
            bg=CARD, fg=GREEN, selectcolor=BG3,
            activebackground=CARD, activeforeground=ACCENT,
            highlightthickness=0,
            font=("Consolas", 8, "bold"),
            command=self._sync_manual_toggle,
        )
        self.manual_enabled_chk.pack(side="left")

        mf = tk.Frame(i, bg=CARD)
        mf.pack(fill="x", pady=(10, 0))


        self.manual_place_btn = tk.Button(mf, text="＋ PLACE MANUAL TRADE",
            bg=ACCENT2, fg="#000000", font=("Consolas", 9, "bold"),
            relief="flat", padx=12, pady=6, cursor="hand2",
            activebackground=ACCENT,
            command=self._place_manual_trade)
        self.manual_place_btn.pack(side="left")

        self.manual_close_btn = tk.Button(mf, text="!! CLOSE MANUAL TRADE",
            bg=RED, fg="#ffffff", font=("Consolas", 8, "bold"),
            relief="flat", padx=10, pady=6, cursor="hand2",
            activebackground="#cc224f",
            command=self._close_all_manual_trades_now)
        self.manual_close_btn.pack(side="left", padx=(8, 0))

        self.manual_status_var = tk.StringVar(value="MANUAL: IDLE")
        tk.Label(i, textvariable=self.manual_status_var, bg=BG, fg=TEXT3,
                 font=("Consolas", 8, "bold")).pack(anchor="w", pady=(8, 0))
    ...
    def _on_symbol_changed(self, event=None):
        if running:
            self.log("Stop engine before changing symbols")
            return
        self._maybe_start_preview_feeds()
    ...
    def _close_leg_verified(self, pair, leg_key, reason):
        leg = pair[leg_key]
        ticket = leg.get("ticket")
        if not ticket:
            return True
        ...
        self.log(f"Pair {pair['id']} closing {leg['label']} #{ticket} ({reason})")
        result = close_order(
            leg["path"], ticket, leg["symbol"], leg["dir"], leg["lot"],
            magic=pair["magic"], comment="martingale_close",
        )

        ok = trade_result_ok(result)
        if ok:
            self.log(f"Pair {pair['id']} {leg['label']} close sent #{ticket}")
        else:
            self.log(f"Pair {pair['id']} {leg['label']} close FAILED: {trade_result_message(result)}")
        return ok
    ...
    def _close_pair_verified(self, pair, reason, clear_on_success=True):
        ...
        # Close master + slave in parallel for maximum speed
        results = [False, False]
        def _close_leg(idx, leg_key):
            results[idx] = self._close_leg_verified(pair, leg_key, reason)
        t1 = threading.Thread(target=_close_leg, args=(0, "master"), daemon=True)
        t2 = threading.Thread(target=_close_leg, args=(1, "slave"), daemon=True)
        t1.start(); t2.start()
        t1.join(); t2.join()

        master_ok, slave_ok = results
        flat = master_ok and slave_ok
    ...
    def _close_all_trades(self):
        """Close EVERY open position in both terminals instantly."""
        def worker():
            global running
            running = False
            self.log("CLOSE ALL: closing all positions in both terminals")
            self.status_var.set("CLOSING ALL")
            for path, name in [(master_path, "Master"), (slave_path, "Slave")]:
                if not path:
                    continue
                try:
                    with _mt5_session(path):
    ...
    def _refresh_live(self):
        """Get prices + PnL from Rust core — no MT5 calls, ~1ms."""
        try:
            s, _, _ = connect_core_socket(connect_timeout=1.0, read_timeout=2.0)
            s.sendall(b"GET\n")
            data = s.recv(1024).decode().strip()
            s.close()
            if data.startswith("DATA "):
                parts = data[5:].split()
                # DATA ma mb sa sb diff entry_th m_pnl s_pnl c_pnl
                if len(parts) >= 6:
                    self.m_price_var.set(parts[0])  # master_ask
                    self.s_price_var.set(parts[2])  # slave_ask
                    self.diff_var.set(parts[4])     # diff
                    if len(parts) >= 9:
                        self.m_pnl_var.set(f"${parts[6]}")
                        self.s_pnl_var.set(f"${parts[7]}")
                        self.c_pnl_var.set(f"${parts[8]}")
                    if len(parts) >= 11:
                        self.m_spd_var.set(parts[9])
                        self.s_spd_var.set(parts[10])
        except:
            pass
        self.root.after(200, self._refresh_live)
    ...
    def _feed_worker_command(self, role, path, symbol):
        worker_script = APP_DIR / "mt5_price_worker.py"
        port = get_core_port()
        common_args = [
            "--role", role,
            "--path", path,
        ...
        if getattr(sys, "frozen", False):
            return [sys.executable, "--feed-worker", *common_args]
        if worker_script.exists():
            return [sys.executable, str(worker_script), *common_args]
        raise FileNotFoundError("mt5_price_worker.py not found")

    def _current_feed_specs(self):
        msym = self.master_sym_var.get()
        ssym = self.slave_sym_var.get()
        if not master_path or not slave_path or not msym or not ssym:
            return None
        return (
            ("MASTER", master_path, msym),
            ("SLAVE", slave_path, ssym),
        )

    def _feed_workers_alive(self):
        return bool(self.feed_procs) and all(proc.poll() is None for proc in self.feed_procs)

    def _maybe_start_preview_feeds(self):
        try:
            specs = self._current_feed_specs()
            if not specs:
                return
            self._start_feed_workers(specs)
        except Exception as e:
            self.log(f"Preview feed start failed: {e}")

    def _start_feed_workers(self, specs=None):
        specs = specs or self._current_feed_specs()
        if not specs:
            raise RuntimeError("Select both terminals and symbols first")
        if self.feed_specs == specs and self._feed_workers_alive():
            return

        self._stop_feed_workers()
        flags = getattr(subprocess, "CREATE_NO_WINDOW", 0)

        for role, path, symbol in specs:
            cmd = self._feed_worker_command(role, path, symbol)
            ...
            self.feed_procs.append(proc)
            self.log(f"{role} feed worker started for {symbol}")
        self.feed_specs = specs

    def _stop_feed_workers(self):
        procs = list(getattr(self, "feed_procs", []))
        self.feed_procs = []
        self.feed_specs = None
        for proc in procs:
            ...
            try:
                ...
                try:
                    ...
                    pass
        try:
            s, _, _ = connect_core_socket(connect_timeout=0.3, read_timeout=0.5)
            s.sendall(b"RESET\n")
            s.recv(128)
            s.close()
        except Exception:
    ...
    def _start(self):
        ...
        try:
            ...
            self.log(f"ERROR: Cannot start feed workers - {e}")
            return
        running = True
        self.status_var.set("RUNNING")
        self.start_btn.config(state="disabled")
    ...
    def _arb_loop_protected(self):
        ...
        try:
            s, core_host, core_port = connect_core_socket(connect_timeout=5.0, read_timeout=5.0)
        ...
        # Push config to Rust
        try:
            s.sendall(f"CONFIG {cfg['entry_th']} {cfg['exit_th']}\n".encode())
            ack = s.recv(128).decode().strip()
            self.log(f"Rust config ACK: {ack}")
        except Exception as e:
            self.log(f"Rust config failed: {e}")
        ...
        while running:
            ...
            try:
                # Read latest worker-fed prices/PnL from Rust.
                s.sendall(b"GET\n")
                data = s.recv(1024).decode().strip()
                ...
                diff = float(parts[4])
                combined_pnl = float(parts[8])
                ms = int(float(parts[9]))
                ss = int(float(parts[10]))
                signal = " ".join(parts[11:]) if len(parts) > 11 else "SIGNAL WATCH"


                # Send prices to Rust — it calculates signal and responds

                # PnL auto-close check
                try:
                    pnl_th = float(self.pnl_threshold_var.get())
                    if pnl_th > 0 and abs(combined_pnl) > pnl_th:
                        self.log(f"PnL {combined_pnl:.2f} exceeds threshold ${pnl_th} -> closing ALL trades")
                ...
                if signal == "SIGNAL EXIT":
                    if pair:
                        self.log(f"Rust EXIT signal diff={diff:.5f}")
                        self._close_pair_verified(pair, "rust exit")
                    continue

                if signal in ("SIGNAL BUY_A_SELL_B", "SIGNAL SELL_A_BUY_B"):
                    if pair:
                    ...
                    if ss > cfg["s_spd"]:
                        ...
                        continue
                    self.log(f"Rust ENTRY signal diff={diff:.5f}")
    ...
    def _sync_manual_toggle(self):
        """
        Requirement: OFF disables ONLY “＋ PLACE MANUAL TRADE”.
        Existing manual pairs keep running (monitor threads are not stopped).
        """
        enabled = bool(self.manual_enabled_var.get())
        if enabled:
            self.manual_place_btn.config(state="normal")
        else:
            self.manual_place_btn.config(state="disabled")

    def _manual_next_pair_id(self):
        with self.state_lock:
            self.manual_pair_seq += 1
            return self.manual_pair_seq

    def _set_manual_status(self, text):
        self.manual_status_var.set(text)

    def _manual_any_open(self):
        with self.state_lock:
            return any(p.get("status") in ("OPENING", "OPEN", "IN TRADE") for p in self.manual_pairs.values())

    def _close_all_manual_trades_now(self):
        """
        Requirement: “!! CLOSE MANUAL TRADE” closes ALL active manual pairs.
        """
        with self.state_lock:
            pair_items = list(self.manual_pairs.items())

        if not pair_items:
            self.log("MANUAL: no manual trades to close")
            self._set_manual_status("MANUAL: IDLE")
            return

        def worker(pair_id, pair):
            try:
                magic = pair.get("magic", MANUAL_MAGIC_BASE + pair_id)
                m = pair.get("master") or {}
                s = pair.get("slave") or {}
                m_ticket = m.get("ticket")
                s_ticket = s.get("ticket")

                self.log(f"MANUAL {pair_id} CLOSE NOW triggered (magic={magic})")
                self._set_manual_status(f"MANUAL: CLOSING #{pair_id}")

                if m_ticket and is_position_open(m["path"], m_ticket):
                    close_order(
                        m["path"], m_ticket, m["symbol"], m["dir"], m["lot"],
                        magic=magic, comment="manual_close",
                    )
                if s_ticket and is_position_open(s["path"], s_ticket):
                    close_order(
                        s["path"], s_ticket, s["symbol"], s["dir"], s["lot"],
                        magic=magic, comment="manual_close",
                    )

                time.sleep(0.6)
                master_closed = (not m_ticket) or (not is_position_open(m["path"], m_ticket))
                slave_closed = (not s_ticket) or (not is_position_open(s["path"], s_ticket))

                with self.state_lock:
                    pair["status"] = "CLOSED" if (master_closed and slave_closed) else "CLOSE_FAILED"
                    if pair["status"] == "CLOSED":
                        self.manual_pairs.pop(pair_id, None)

                if master_closed and slave_closed:
                    self.log(f"MANUAL {pair_id} closed OK (CLOSE NOW)")
                    self._set_manual_status(f"MANUAL: CLOSED #{pair_id}")
                else:
                    self.log(f"MANUAL {pair_id} close FAILED (CLOSE NOW)")
                    self._set_manual_status(f"MANUAL: CLOSE FAILED #{pair_id}")
            except Exception as e:
                self.log(f"MANUAL {pair_id} close now error: {e}")
                with self.state_lock:
                    self.manual_pairs.pop(pair_id, None)
                self._set_manual_status("MANUAL: ERROR")

        for pair_id, pair in pair_items:
            threading.Thread(target=worker, args=(pair_id, pair), daemon=True).start()

    def _place_manual_trade(self):
        # Independent of engine running/open pair
        if not master_path or not slave_path:
            self.log("MANUAL: Select both terminals!")
            return
        if not self.master_sym_var.get() or not self.slave_sym_var.get():
            self.log("MANUAL: Select both symbols!")
            return

        try:
            manual_exit_th = float(self.manual_exit_diff_var.get())
        except Exception:
            self.log("MANUAL: invalid close diff")
            return

        # Requirement: use manual_lot_var for manual orders
        try:
            manual_lot = float(self.manual_lot_var.get())
        except Exception:
            self.log("MANUAL: invalid manual lot")
            return
        ...
        cfg = {
            ...
            "ssym": self.slave_sym_var.get(),
            "lot": manual_lot,
            "m_dir": mt5.ORDER_TYPE_BUY if self.manual_master_dir_var.get() == "BUY" else mt5.ORDER_TYPE_SELL,
            "s_dir": mt5.ORDER_TYPE_BUY if self.manual_slave_dir_var.get() == "BUY" else mt5.ORDER_TYPE_SELL,
            "exit_th": manual_exit_th,
        }

        def worker(pair_id, magic, pair_state):
            try:
                self._set_manual_status(f"MANUAL: OPENING #{pair_id}")
                self.log(f"MANUAL {pair_id} opening (magic={magic})")

                master_comment = "manual_open"
                slave_comment = "manual_open"

                m = pair_state["master"]
                m_result = place_order(
                    m["path"], m["symbol"], m["dir"], m["lot"],
                    magic=magic, comment=master_comment,
                )
                if not trade_result_ok(m_result):
                    self.log(f"MANUAL {pair_id} Master FAILED: {trade_result_message(m_result)}")
                    with self.state_lock:
                        pair_state["status"] = "OPEN_FAILED"
                    self._set_manual_status("MANUAL: FAILED")
                    return

                m["ticket"] = trade_result_ticket(m_result)
                self.log(f"MANUAL {pair_id} Master verified #{m['ticket']}")

                s = pair_state["slave"]
                s_result = place_order(
                    s["path"], s["symbol"], s["dir"], s["lot"],
                    magic=magic, comment=slave_comment,
                )
                if not trade_result_ok(s_result):
                    self.log(f"MANUAL {pair_id} Slave FAILED: {trade_result_message(s_result)} -> closing master")
                    # Close master only
                    if m.get("ticket"):
                        close_order(
                            m["path"], m["ticket"], m["symbol"], m["dir"], m["lot"],
                            magic=magic, comment="manual_close",
                        )
                    with self.state_lock:
                        pair_state["status"] = "OPEN_FAILED"
                ...
                s["ticket"] = trade_result_ticket(s_result)
                with self.state_lock:
                    pair_state["status"] = "IN TRADE"

                self._set_manual_status(f"MANUAL: IN TRADE #{pair_id}")
                self.log(f"MANUAL {pair_id} fully open -> waiting close diff < {pair_state['exit_th']}")

                # Requirement: per-pair monitoring independent; diff close + one-leg-flat force close
                while True:
                    with self.state_lock:
                        mp = self.manual_pairs.get(pair_id)
                    if not mp:
                        return
                    ...
                    # One-leg-flat safeguard
                    m_open_now = is_position_open(m["path"], m.get("ticket"))
                    s_open_now = is_position_open(s["path"], s.get("ticket"))
                    if m_open_now != s_open_now:
                        self.log(
                            f"MANUAL {pair_id} one-leg flat detected -> forcing other side close"
                        )
                        if m_open_now:
                            close_order(
                                m["path"], m["ticket"], m["symbol"], m["dir"], m["lot"],
                        ...
                        if s_open_now:
                            close_order(
                                s["path"], s["ticket"], s["symbol"], s["dir"], s["lot"],
                                magic=magic, comment="manual_close",
                            )
                        time.sleep(0.6)
                        with self.state_lock:
                            self.manual_pairs.pop(pair_id, None)
                        self._set_manual_status("MANUAL: CLOSED (one-leg)")
                        return

                    # Diff-based auto-close
                    if diff < pair_state["exit_th"]:
                        self.log(
                            f"MANUAL {pair_id} EXIT diff={diff:.5f} < {pair_state['exit_th']} -> closing"
                        )
                        if m_open_now:
                            close_order(
                                m["path"], m["ticket"], m["symbol"], m["dir"], m["lot"],
                        ...
                        with self.state_lock:
                            self.manual_pairs.pop(pair_id, None)
                        self._set_manual_status("MANUAL: CLOSED (diff)")
                        return

                    time.sleep(0.4)

            except Exception as e:
                self.log(f"MANUAL {pair_id} monitoring error: {e}")
                with self.state_lock:
                    self.manual_pairs.pop(pair_id, None)
                self._set_manual_status("MANUAL: ERROR")

        pair_id = self._manual_next_pair_id()
        magic = MANUAL_MAGIC_BASE + pair_id
        pair_state = {
            "id": pair_id,
            "magic": magic,
            "status": "OPENING",
        ...
        threading.Thread(target=worker, args=(pair_id, magic, pair_state), daemon=True).start()
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--feed-worker":
        from mt5_price_worker import parse_args, run_worker
